...
namespace robot_basic_calc
{
	...
	inline Mat3 MatrixExp3vec(const Vec3& vec)
	{
	...
	Mat4 MatrixExp6(const Mat4& se3mat) {
		if (NearZero(se3ToVec(se3mat).norm()))
			return I4;
		...
		if (NearZero(omgtheta.norm())) {
			...
			Mat3 expExpand = I3 * theta + (1 - std::cos(theta)) * omgmat + ((theta - std::sin(theta)) * (omgmat * omgmat));
	...
	inline Mat4 MatrixExp6vec(const Vec6& screw)
	{
		return MatrixExp6(VecTose3(screw));
	}
	...
	Mat4 MatrixLog6(const Mat4& T) {
		...
		if (NearZero(omgmat.norm())) {
			...
			Mat3 logExpand1 = Eigen::MatrixXd::Identity(3, 3) - omgmat / 2.0;
	...
	inline Vec6 MatrixLog6vec(const Mat4& T){
		return se3ToVec(MatrixLog6(T));
	}


	Eigen::Matrix<double, 6, -1> JacobianSpace(const Eigen::Matrix<double, 6, -1>& Slist, const VecX& thetaList) {

		Eigen::Matrix<double, 6, -1> Js = Slist;
		Mat4 T = I4;
